...
<div class="card">
	<div class="card-block card-block-header">
		<h4 class="card-title"><i class="fa fa-child"></i> Приветик <?php echo $_SESSION['user']['benutzer']; ?>!</h4>
	</div>
</div>
...
<?php
	if($user_right['right_hp_ticket_system'] == $mysql_keys['right_hp_ticket_system']) {
		...
		<div class="card">
			<div class="card-block card-block-header">
				<h4 class="card-title"><i class="fa fa-ticket"></i> Тикеты</h4>
			</div>
			<div class="card-block">
...
<script>
 	$(function ()
	{
		...
		for (var instanz in arrayOfSlots)
		{
			...
			datasets['labels']												=	["<?php echo $language['client']; ?> Онлайн", "Доступных слотов"];
